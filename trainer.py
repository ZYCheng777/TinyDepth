...
#python train.py --model_name Tiny-Depth --scales 0 --png --log_dir models --data_path /home/ace/data/kitti_dataset/
...
class Trainer:
    def __init__(self, options):
        ...
        self.config = get_config(options)
        self.num_ch_enc = [64, 64, 128, 160, 320]
        ...
        num_params_dd = sum([np.prod(p.size()) for p in self.models["depth"].parameters()])


        print("Total number of parameters: {}".format(num_params_de+num_params_dd)) #参数量
    ...
    def process_batch(self, inputs, epoch):
        ...
        losses = self.compute_losses(inputs, outputs, epoch)
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                outputs[("color_"
                         "HiS", frame_id, scale)] = F.grid_sample(
    ...
    def compute_normal_loss(self, pred_disp, target_disp, intrinsics):
        ...
        normal_loss = (pred_depth_normal - target_depth_normal).abs()
    ...
    def compute_reprojection_loss_disp(self, pred, target):  #pe photometric error
        """Computes reprojection loss between a batch of predicted and target images
        """
    ...
    def compute_losses(self, inputs, outputs, epoch):
        ...
        for scale in self.opt.scales:
            ...
            disp_HiS = outputs["out_HiS"][("disp", 0)]
            ...
            disp_LoS = outputs["out_LoS"][("disp", 0)]
    ...
    def compute_ambiguity_mask_HiS(self, inputs, outputs,
        ...
        target_ambiguity = self.extract_ambiguity(inputs[("color_HiS", 0, src_scale)])

        reproj_ambiguities = []
        for f_i in self.opt.frame_ids[1:]:
            src_ambiguity = self.extract_ambiguity(inputs[("color_HiS", f_i, src_scale)])

            reproj_ambiguity = F.grid_sample(
                src_ambiguity, outputs[("sample_HiS", f_i, scale)],
    ...
    def compute_ambiguity_mask_MiS(self, inputs, outputs,
                               reprojection_loss, scale):
        src_scale = scale if self.opt.v1_multiscale else 0
        min_reproj, min_idx = torch.min(reprojection_loss, dim=1)

        target_ambiguity = self.extract_ambiguity(inputs[("color_MiS", 0, src_scale)])

        reproj_ambiguities = []
        for f_i in self.opt.frame_ids[1:]:
            src_ambiguity = self.extract_ambiguity(inputs[("color_MiS", f_i, src_scale)])

            reproj_ambiguity = F.grid_sample(
                src_ambiguity, outputs[("sample_MiS", f_i, scale)],
                padding_mode="border", align_corners=True)
            reproj_ambiguities.append(reproj_ambiguity)
    ...
    def compute_ambiguity_mask_LoS(self, inputs, outputs,
                               reprojection_loss, scale):
        src_scale = scale if self.opt.v1_multiscale else 0
        min_reproj, min_idx = torch.min(reprojection_loss, dim=1)

        target_ambiguity = self.extract_ambiguity(inputs[("color_LoS", 0, src_scale)])

        reproj_ambiguities = []
        for f_i in self.opt.frame_ids[1:]:
            src_ambiguity = self.extract_ambiguity(inputs[("color_LoS", f_i, src_scale)])

            reproj_ambiguity = F.grid_sample(
                src_ambiguity, outputs[("sample_LoS", f_i, scale)],
                padding_mode="border", align_corners=True)
            reproj_ambiguities.append(reproj_ambiguity)
