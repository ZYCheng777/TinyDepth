...
from layer import transformation_from_parameters
...
def evaluate(opt):
    ...
    #pose_encoder = networks.build_model(config, 2)
    pose_encoder.load_state_dict(torch.load(pose_encoder_path))

    pose_decoder = networks.PoseDecoder(pose_encoder.num_ch_enc, 1, 2)
    #pose_decoder = networks.PoseDecoder(num_ch_enc, 1, 2)
    ...
    with torch.no_grad():
        for inputs in dataloader:
            ...
            all_color_aug = torch.cat([inputs[("color_MiS", i, 0)] for i in opt.frame_ids], 1)
